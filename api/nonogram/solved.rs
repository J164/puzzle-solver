use puzzle_utils::{image_to_png_bytes, print_nonogram, print_nonogram_solution};
...
pub async fn handler(req: Request) -> Result<Response<Body>, Error> {
    ...
    match payload {
        ...
        Ok(Some(PrintOptions {
            ...
            let unsolved = match print_nonogram(width, height, &col, &row) {
                Ok(unsolved) => unsolved,
                Err(nonogram_error) => return bad_request(nonogram_error.to_string()),
            };

            let nonogram = match print_nonogram_solution(width, height, unsolved, &grid) {
